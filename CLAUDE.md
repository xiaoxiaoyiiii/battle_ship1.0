...
> 最后更新：2026-09-22（大师 AI 第 4 批：四条交互通道齐备，69.5%；34 张卡逐张量过）。
...
36. **"改前跑 2000 局、改后再跑 2000 局，比两个数字"是在读噪声**：大师 AI 的胜率
    在 ±1.5 个百分点内高频抖动，我据此记下过 71.2% 并写进文档，切开种子序列
    成两个独立 4000 局半样本后真实值是 **69.5%**（69.2 / 69.7 互相吻合）。
    → 判定一个改动：要么样本翻几倍，要么**两个独立半样本互相对上**；
    单张卡的 ±0.6 这类差值只能当"没有证据说明它有害"，不能当结论。
    同批还因此回退了自己写的"更讲道理"的启发式（实测没有变好）。
