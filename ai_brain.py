...
CARD_BASE_VALUE = {
    ...
    '增援': 78,
    '命运骰子': 76,        # 期望为正但方差大
    '无忧梦呓': 60,        # 判定但**延迟兑现**（对方的回合才开始拼点）：
                           # 15/36 概率锁死对方这一回合的攻击、15/36 让对方弃一张、
                           # 6/36 平局无事 —— 期望为正，但要等半个大回合才到账。
                           # （不进大师卡池，理由见 server.py `_MASTER_ENABLED_CARDS`
                           #   旁边那条；这里仍然给分是因为价值表必须覆盖每一张能
                           #   摸到的卡，守卫见 tests/test_ai_brain.py）
    '极限增援': 75,
    '绝处逢生': 75,
...
#
# ⚠️ **保留原值 50**（2026-09-23 本批**试过 65，实测在同一批改动的池子上更差**）：
#
#   池子 A（**含**「平等条约」的旧池，5000 局 × 3 个独立种子）：
#       阈值 50    69.4% [68.1,70.6]   70.9% [69.6,72.2]   68.9% [67.6,70.2]
#       阈值 65    70.1% [68.8,71.4]   71.2% [69.9,72.4]   69.2% [67.9,70.5]  ⇒ +0.5
#
#   池子 B（把「平等条约」移出池子之后，= 本批最终的池子，5000 局 × **4** 个种子）：
#       阈值 50    74.9% [73.7,76.1]   75.9% [74.7,77.0]   74.0% [72.7,75.2]   74.3% [73.1,75.5]
#       阈值 58    74.9% [73.7,76.1]   75.9% [74.7,77.0]   74.0% [72.7,75.2]   74.3% [73.1,75.5]
#       阈值 62    74.9% [73.7,76.1]   75.8% [74.6,76.9]   73.9% [72.7,75.1]   74.2% [73.0,75.4]
#       阈值 65    74.1% [72.8,75.3]   75.1% [73.9,76.3]   72.9% [71.6,74.1]   73.3% [72.0,74.5]  ⇒ −0.8~−1.1
#       阈值 72    69.4% [68.1,70.6]   70.3% [69.0,71.5]   67.9% [66.6,69.2]   69.3% [68.0,70.5]
#
#   ⇒ **"最优阈值"随卡池一起变**：65 之所以在旧池上更好，是因为它剔掉的正是
#     「平等条约（58）」「加百列之光（60）」那一档 58~65 分的低收益牌；
#     而「平等条约」一旦移出池子，这一档就空了 —— 65 变成纯粹地在砍好牌。
#     **这就是"单独调一个旋钮"最容易骗人的地方：它的最优值依赖别的改动。**
#   ⇒ 所以本批**不动这个常量**，并把上面两张表都留在注释里，
#     下一批要动它必须在**当时的卡池**上重跑多半样本（`tools/experiment_threshold.py`）。
#
#   两个方向的边界（在本批最终池子上量过）：降到 0（有牌就出）会掉分，
#   升到 72 会掉到 ~69% —— 它是一个**有最优点**的量，不是"越少出牌越好"。
PLAY_THRESHOLD = 50
