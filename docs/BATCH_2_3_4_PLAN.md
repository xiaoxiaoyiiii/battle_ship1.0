...
## 12. 追加批：等级 / 经验系统 + 匹配等待界面的对手信息（2026-09-17）

作者原话：① 匹配成功的 5 秒等待界面里显示**对方的称号和标签**；② 等级系统 ——
**满级 100**、他的账号 **101 级**、要有**可视化的经验条**能看自己的经验和等级；
③ 每场结束显示**本局获得多少经验**，做成**滚动的条**，升级就播"条子重置"的动画再继续加。
"具体怎么做你来定夺"。

### 12.1 设计（`leveling.py` = 唯一一份规则）

| 项 | 取值 | 说明 |
| --- | --- | --- |
| 曲线 | 到达 L 级需累计 `30*(L-1) + 5*(L-1)²` | L10=675 ｜ L50=13475 ｜ L100=51975 ｜ L101=53000 |
| 满级 | **普通 100**，持 `level_101` 特权 **101** | 与"外观全解锁"同一套特权机制（`user_perks`） |
| 每局经验 | 参与 **10** + 胜利 **15** + 每击沉 **2** + 零伤获胜 **10** + 三分钟内获胜 **5** | 输也有参与分；额外奖励**只在获胜时**给 |
| 存储 | 新表 `user_xp(user_id, xp, updated_at)` | **只存 xp，不存 level**：等级一律由 `level_from_xp()` 推出，避免"经验涨了等级没更新" |

**关键契约：动画分段由服务端下发**。结算时 `xp_gained` 事件带
`segments[{level, from, to, need, ratio_from, ratio_to, level_up}]` ——
前端只按段播滚动条，**不重算曲线**（两份实现必然漂移，第 2 批的教训）。
升级段的 `to` 必须正好等于 `need`，前端靠"这一段填满"来播升级动画；
升级后条子清空（下一段 `from=0`）继续加。满级后仍记经验但不再升级（以后提高上限不用迁移数据）。

### 12.2 落点

- **匹配等待界面**：`#opponent-chips` 显示对手的 `Lv.N` + 称号 + 标签，
  数据取自公开的 `/user_stats`（与名片同一来源）；`/user_stats` 新增 **`tag_names`**
  （服务端直接给中文名，省得前端再去够那个函数作用域里的映射，见 12.3 第 1 条）。
- **首页**：`#my-level-strip`（Lv + 经验条 + `x / y`），登录后异步取 `/api/profile`。
- **名片**：`Lv.N` 改为**服务端等级**（原先是按场次估算的假等级），下方加 `#profile-view-xp` 经验条。
- **结算**：`#xp-gain-panel` —— 本局 `+N` 经验、经验明细、滚动条、`升级！Lv.N → Lv.N+1` 闪光。
- **接口**：`/api/profile`、`/user_stats`、`/api/leaderboard` 三处都下发等级
  （排行榜是逐行 `level`，经验用 `get_xp_map` 一条 SQL 批量取）。
- **结算收口**：`_finalize_match` 里新增第 ④ 步 `_grant_match_xp`（与战绩/统计/徽章同一个出口），
  人机对局不发经验（`count_stats` 早拦住了）。

### 12.3 实施中踩到的坑

1. **★ 一天里第二次踩「函数作用域里的东西被模块级代码引用」**：
   `showOpponentChips()` 是模块级的，却调了声明在**名片渲染函数内部**的
   `profileTitleName()` / `profileTagNames()` → 运行时 `ReferenceError`，
   而且跑在 `.then()` 里**连异常都不显眼**（表现只是"名字有了、标签一个都不显示"）。
   上午已经因为 `PROFILE_BADGE_GLYPH` 栽过一次，这是同一个坑。
   **修法**：让服务端把 `tag_names` 直接下发，前端不再去够那个作用域。
   👉 规矩：**写模块级新函数前，先确认它要用的每个东西都声明在模块级**；
   `node --check` 抓不到，只有真跑一遍才会暴露。
2. **测试期望写错三次（都是"我没搞清产品口径"而不是产品错）**：
   · 排行榜**只收真打过一局的账号** → 给新号设经验后断言"它应在榜上"必然找不到；
   · 结算经验**双方都发**（输方也有参与分），我却断言"只有胜者收到"；
   · 数"调用点"时把 `def _grant_match_xp(room, rows)` 这行也算进去了（contains 了同样的前缀）。
   👉 三次都是**先看代码确认口径，再改期望**，没有为了让测试变绿去改产品。
3. **工具的两处自身问题**：`perkvip` 第二次跑时"首胜"已解锁 → 不再有"刚解锁"提示（**这本来就是对的**，
   所以脚本每次跑之前要重置该账号）；`#xp-levelup` 的可见性不能用"有没有某个类"判，
   要用 `hidden` 属性 + 尺寸（沿用第 4 批的教训）。

### 12.4 实测

- pytest：**1199 passed**（新增 `tests/test_leveling.py` 27 条：曲线锚点、段自洽性、
  经验只增不减、三处接口口径、结算事件）
- `tools/level_check.mjs`（新建，两个独立浏览器、真实结算路径）：**25 项全绿** ——
  首页 Lv.1「30 / 35」→ 打一局 +35 → **滚动** + **升级！Lv.1 → Lv.2** → 最终 Lv.2「35 / 35」→
  首页条同步成 Lv.2；等待界面显示对手 **Lv.101 + 称号 + 两个标签**
- 受影响工具复跑：`achievements_check` 35 ｜ `profile_card_check` 57 ｜
  `profile_leaderboard_check` 47 ｜ `ui_layout_check` 100 ｜ `unlock_notice_check` 27，全部 **0 FAIL**
