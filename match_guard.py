...
def pick_pair(queue: list) -> tuple:
    ...
    返回 `(i, j, assessed)`：
      ...
      · `assessed` —— **这一对是否"干净"**：
          True  = 按规避规则挑出来的最优对
          False = 走到兜底段了（只剩软规避的对象可配，仍然配，但该局不结算排位分）

    算法：
      ① 遍历所有 **mode 相同** 且 **未被硬拦** 的组合，取 `pair_score` 最高的；
      ② 若最高分 ≥ `SCORE_BASE`（= 没有任何软规避扣分）→ 干净对，`assessed=True`；
         否则若存在**干净对**（`pair_score == SCORE_BASE` 且等了足够久）也正常返回；
...
def should_settle_ranked(assessed: bool) -> bool:
    """兜底配出来的那一对该不该结算排位分。

    ⚠️ 兜底对（`assessed=False`）**不结算排位分** —— 这是整个设计的关键：
    宁可让"同一 IP 的两个人"能开上一局娱乐，也不给他们刷分的机会。
    他们想拿分，就等队列里出现第三个人（那时会配成干净对）。
    """
