...
class PlayerShip:
    ...
    def __init__(self, id:str, positions:list[Position], hits:list[Position]):
...
class GameRoom:
    ...
    def __init__(self, room_id):
        ...
        self.pending_magic = ""  # 待处理的魔法卡（等待对方是否使用失灵）
...
def handle_attack(data):
    ...
    for i, ship in enumerate(defender_ships):
        if {'x': target_x, 'y': target_y} in ship.positions:
            ...
            # 检查攻击者是否有强制击杀效果
            has_forced_kill = room.players[attacker_id].effect_flags.get('forced_kill', 0) > 0
            ...
            if has_forced_kill:
                ...
                # 检查饮血效果
                if room.players[attacker_id].effect_flags.get('vampire'):
                    room.draw_card(attacker_id)
                    emit('message', {'text': '饮血效果发动，抽一张卡'}, to=attacker_id)

                # 检查绝处逢生效果
                if room.players[attacker_id].effect_flags.get('last_stand'):
                    # 直接获胜
...
def enter_battle_phase(data):
    ...
    if room.current_attacker == player_id and room.current_phase == 'preparation':
        ...
        # 检查是否有攻击次数翻倍效果
        if room.players[player_id].effect_flags.get('double_attacks'):
            # 翻倍当前攻击次数
