...
window.magicCards = [
    ...
    { name: "五险一金", speed: 2, type: "普通", description: "这一回合自己的攻击次数第一次用尽时，若自己未曾对对方造成一点伤害，则自己的攻击次数再加3。" },
    ...
    { name: "饮血", speed: 2, type: "普通", description: "可在击沉对方一艘战舰后选择使用。发动时立即为这一艘沉船摸一张牌；接下来自己的攻击，每击杀一艘船，自己摸一张牌。" },
    ...
    { name: "命运骰子", speed: 1, type: "判定", description: "进行一次摇骰子判定。如果摇到了1那么复活对方的一艘战舰，如果没有就不用复活；如果摇到了2那么让对方抽一张牌；如果摇到了3那么双方都要弃掉一张手牌；如果摇到了4那么自己抽两张牌；如果摇到了5那么复活自己的至多两艘战舰；如果摇到了6那么让对手牺牲两艘战舰。" },
    { name: "无忧梦呓", speed: 1, type: "判定", description: "这张牌通过后，在对方的下一个回合开始时双方进行一次摇骰子拼点判定。如果对方的点数小于自己，那么对方在这个回合的攻击次数恒定为0；如果对方的点数大于自己，那么对方必须弃置一张手牌（没有手牌则跳过）；如果双方点数相同，那么什么都不发生。这个效果只会触发一次。" },
    { name: "亡羊补牢", speed: 1, type: "普通", description: "当弃牌区有卡牌时可以发动。拿出弃牌区最新的n张卡牌（n为双方的最大船数），选定其中一张加入自己的手牌，其余的回归弃牌堆。" },
    { name: "守株待兔", speed: 1, type: "普通", description: "这张牌只会在当前大回合生效。选定自己的一艘船为其添加陷阱效果；如果这艘船死亡，那么对方需要自行牺牲两艘船；如果对方的船数为1那么也需要牺牲唯一的一艘船。" },
    { name: "卧薪尝胆", speed: 1, type: "普通", description: "当自己的船数小于对方的场合可以发动。为自己的所有船添加护盾效果。" },
    { name: "滥竽充数", speed: 3, type: "普通", description: "这张牌仅可在当前大回合内生效。补充满自己的船数并自己选择放置被补充的船。这些在当前大回合被补充的船将会在大回合结束时强制收回，不会显示沉没。在极限情况下（即剩余的格子数比需要补充的船数少的时候）就尽可能多的补充就可以。" }
];
